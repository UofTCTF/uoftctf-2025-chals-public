...
def parse_data():
    f = open("out").read().splitlines()
    f = f[3:]
    dat = []
    for i in divide_chunks(f, 4):
        if int(i[0].split()[1]) == 2: # if a decryption
            enc = bytes.fromhex(i[1].split()[-1])
            dec = bytes.fromhex(i[2].split()[-1])
            time = float(i[3].split()[2])
            dat.append((enc, dec, time))
    return dat


def get_prediction():
    times = [d[2] for d in dat]
    key = []
...
print(len(dat))
pred=get_prediction()
print([[hex(i) for i in b] for b in pred])

for key in tqdm(itertools.product(*pred), total = prod(map(len, pred))):
    key = bytes(key)
    key = reverse_key_schedule(key, 10)
    chiph = AES.new(key, AES.MODE_ECB)
    if chiph.decrypt(verifyEnc) == verifyDec:
